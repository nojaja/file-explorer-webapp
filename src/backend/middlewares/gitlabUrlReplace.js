...
export function gitlabUrlReplaceMiddleware(req, res, next) {
  ...
  // GitLab URLの情報を取得
  const gitlabContainerUrl = global.authConfig.gitlab.GITLAB_URL_INTERNAL;//  'http://gitlab:8929';
  const gitlabBrowserUrl = global.authConfig.gitlab.GITLAB_URL;//  'http://localhost:8929';
  ...
  const processUrl = (url) => {
    ...
    // Hydra Authの置換
    if (url.includes(gitlabContainerUrl)) {
